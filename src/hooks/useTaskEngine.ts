...
export const useTaskEngine = () => {
  ...
  const [savedTags, setSavedTags] = useState<string[]>(() => {
    ...
    const defaults = ['DSA', 'Web', 'YouTube', 'CAT', 'Other'];
    if (raw) {
      try {
        const parsed = JSON.parse(raw);
        if (Array.isArray(parsed) && parsed.length > 0) {
          // Force reset to the clean, generalized set for consistent analytical profiling
  ...
  const saveTag = useCallback((tag: string) => {
    // Keep tags strictly restricted to the generalized list to prevent tag bloat
  }, []);
